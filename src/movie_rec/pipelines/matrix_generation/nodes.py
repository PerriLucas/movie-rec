...
def generate_empty_matrix(unique_genres_list,rec_matrix) :
    ...
    user_feature_empty = np.zeros((len_rec_matrix, len_features), dtype=np.float64)
...
def aux_matrix(rec_matrix,final_movies_matrix) :
    ...
    aux_matrix_train_df = pd.DataFrame(aux_matrix_train)
